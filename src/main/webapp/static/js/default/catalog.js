$().ready(function() {
    ...
    //数据校验
    validator($('#addCatalogForm'));
    validator($('#updateCatalogForm'));
...
function init(){
    ...
    $('#dataTable').bootstrapTable({
        ...
        url : "/restaurant/findAllCatalog", // 服务器数据的加载地址
        iconSize : 'outline',
...
window.operateEvents = {   //添加一个按钮在对应的按钮组中写下需要使用的事件
    "click #info":function(e,value,row,index){  //查看按钮事件  row当前行，index当前行的下标
        $("#viewCatalogName").val(row.catalogName);
        $('#viewStatus').find("option").each(function (i,n) {
            if(n.value == row.status){
                n.selected = true;
            }
        })
    ...
    "click #edit":function(e,value,row,index){  //编辑按钮事件  row当前行，index当前行的下标
        $("#id").val(row.id);
        $("#updateCatalogName").val(row.catalogName);
        $('#updateStatus').find("option").each(function (i,n) {
            if(n.value == row.status){
                n.selected = true;
            }
        })
        $('#updateFoodCatalog').modal('show');
    }
}
...
function  btnFunction() {
    ...
    $('#addCatalogBtn').click(function () {
        ...
        //验证输入数据结果
        if($('#addCatalogForm').data('bootstrapValidator').isValid()){
            $.ajax({
                type:"post",
                url:"/restaurant/addCatalog",
    ...
    $('#updateCatalogBtn').click(function () {
        var id = $('#id').val();
        var catalogName = $('#updateCatalogName').val();
        var status = $('#updateStatus').find("option:selected").val();
        $.ajax({
            method:"post",
            url:"/restaurant/updateCatalog",
            data:{"id":id,"catalogName":catalogName,"status":status},
            success:function (result) {
                location.reload()  //刷新当前页面
            },
            error:function () {
